...
namespace GMap.NET
{
   ...
   using Thread=OpenNETCF.Threading.Thread2;
    ...
    public class GMaps : Singleton<GMaps>
    {
        ...
        /// </summary>
        public RasterImagesProvider RasterImageProvider;
        ...
        public MapType[] GetAllLayersOfType(MapType type)
        {
            MapType[] types = null;
            {
                switch (type)
                {
                    case MapType.RasterHybrid:
                        {
                            types = new MapType[2];
                            //types[0] = MapType.GoogleTerrain;
                            types[0] = MapType.RasterMaps;
        ...
        public PureImage GetImageFrom(MapType type, Point pos, int zoom, out Exception result)
        {
            ...
            try
            {
                if (type == MapType.RasterMaps)
                {
                    if (GMaps.Instance.RasterImageProvider == null)
                    {
                        result = new ApplicationException("Cannot use Raster Maps becuase raster image folder has not be initialized.");
                    }
                    ret = GMaps.Instance.RasterImageProvider.GetRasterImage(pos, zoom);
                }
                else
                {
